...
import { generateCapstonePrompt } from "../../core/ai/prompts/capstone-temp";
    ...
    try {
      ...
      const chapterLessons = await Promise.all(
        chapters.map(async (chapter) => {
          ...
          return lessons.map((lesson) => ({ ...lesson, chapterId: chapter.id }));
        }),
      );

      const allLessons: any[] = chapterLessons.flat();
